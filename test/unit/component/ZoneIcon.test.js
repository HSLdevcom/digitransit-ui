...
describe('<ZoneIcon />', () => {
  ...
  it('should add multi-letter classes for multi-character zoneId', () => {
    ...
    expect(container.querySelector('.multi-letter-container')).not.toBe(null);
    expect(container.querySelector('.multi-letter')).not.toBe(null);
  });

  it('should not add multi-letter class for single-character zoneId', () => {
    const { container } = renderWithProviders(<ZoneIcon zoneId="A" />, opts);
    expect(container.querySelector('.multi-letter')).toBe(null);
  });
  ...
  it('should add unknown-container class for unknown zone', () => {
    ...
    expect(container.querySelector('.unknown-container')).not.toBe(null);
  });
