    ...
    <div className="route-stop row">
      ...
      <Link to="/pysakit/#{@props.stop.gtfsId}">
        <div className={"columns small-6 route-stop-name " + @props.mode}>
          {@props.stop.name} {<WalkDistance className="nearest-route-stop" icon="icon_location_with_user" walkDistance={@props.stop.nearestDistance}/> if @props.stop.nearestDistance}<br/>
