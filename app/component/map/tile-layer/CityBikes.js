...
const scaleratio =
  (typeof window !== 'undefined' && window !== null ? window.devicePixelRatio : void 0) || 1;
const citybikeImageSize = 18 * scaleratio;
const availabilityImageSize = 8 * scaleratio;
const notInUseImageSize = 14 * scaleratio;
...
class CityBikes {
    ...
    fetch(`${config.URL.CITYBIKE_MAP}` +
      ...
      return res.arrayBuffer().then(buf => {
        ...
        this.features = (() => {
          ...
          if (vt.layers.stations !== undefined) {
          ...
          return results;
        })().map(_i => vt.layers.stations.feature(_i));
  ...
  fetchAndDrawStatus = (feature, geom) => {
    ...
    if (lastFetch && currentTime - lastFetch <= 30000) {
      return Relay.Store.primeCache({
        query,
      }, callback);
    }
    return Relay.Store.forceFetch({
      query,
    }, callback);
  }
  ...
  addFeature = (feature) => {
    ...
    return this.fetchAndDrawStatus(feature, geom);
  }
