...
class Page extends React.Component
  ...
  render: ->
    ...
    leafletObjs = []
    leafletObjs.push <CircleMarker key="marker_outline" center={lat: stop.lat, lng: stop.lon} radius=13 weight=1 color="#333" opacity=0.4 fillColor="#fff" fillOpacity=1 />
    leafletObjs.push  <CircleMarker key="marker" center={lat: stop.lat, lng: stop.lon} radius=8 weight=7 color={color} opacity=1 fillColor="#fff" fillOpacity=1 />

    <DefaultNavigation className="fullscreen">
      <Map lat={stop.lat+0.0005} lon={stop.lon} zoom={16} leafletObjs={leafletObjs} className="fullscreen" showStops=true skipStops=[stop.id]>
