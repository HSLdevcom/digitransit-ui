...
var RelayQueryPath = (function () {
  ...
  RelayQueryPath.prototype.getPath = function getPath(node, dataID) {
    if (GraphQLStoreDataHandler.isClientID(dataID)) {
      ...
      var root = RelayQuery.Root.build(RelayNodeInterface.NODE, dataID, [idField, typeField], { rootArg: RelayNodeInterface.ID }, this.getName());
  ...
  RelayQueryPath.prototype.getQuery = function getQuery(appendNode) {
    ...
    return RelayQuery.Root.build(rootCall.name, rootCall.value, [child, node.getFieldByStorageKey(ID), node.getFieldByStorageKey(TYPENAME)], _extends({}, node.__concreteNode__.metatada, {
