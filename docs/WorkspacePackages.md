...
| ----------------------- | -------------------------- | ------------------------------------- | ------------------------------------------------------------ | -------------------------------------------- |
| `digitransit-component` | `@digitransit-component/*` | `digitransit-component/packages/*`    | `src/index.jsx` (`src/index.js` for the 2 packages with no JSX: `digitransit-component`, `digitransit-component-abtesting`) | [Rollup](https://rollupjs.org) (compiles JSX) |
...
`.jsx` applies only where the file actually contains JSX — every `component`
package except `digitransit-component`/`digitransit-component-abtesting`
(no JSX), and no `store`/`search-util`/`util` package (none has JSX).
...
- `src/index.js(x)` / `index.js` — the module's implementation, documented
  with [JSDoc](https://jsdoc.app/). This JSDoc is the *only* source of truth
  for the generated `README.md` — write real descriptions, `@param`/
  `@returns`, and an `@example`, not just type annotations.
- `test.js(x)` — real, executable [Vitest](https://vitest.dev) (and, for
  `component`, RTL) tests, run against raw source (`src/index.js(x)`/
...
4. Replace `src/index.js(x)` (or `index.js`) with your implementation, and
   `test.js(x)` with real tests for it (`.jsx` only if it actually contains
   JSX).
...
7. Add tests to CI simply by existing — `yarn workspace-packages-test` (part
   ...
   (`digitransit-<family>/packages/*/test.js`, or `test.{js,jsx}` for
