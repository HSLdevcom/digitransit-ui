...
Components are built with [Rollup](https://rollupjs.org), which bundles
each module's `index.js` into the gitignored `lib/` directory. Build all
workspace packages by running, from the repository root:
```
$ yarn build-components
```
(this also runs as part of `yarn setup`/CI). To build a single package,
navigate to its directory and run `yarn build`.

## Compiling tests with Babel

Tests (`test.js`) use syntax that needs to be compiled with
[Babel](babeljs.io) before running. Navigate to the component's directory
and run
```
$ yarn build:test
```
it will create `test.generated` - this file has been generated from
`test.js`, and is what `yarn test` (mocha) actually runs.
